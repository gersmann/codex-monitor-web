...
async function fileExistsAtPath(targetPath: string) {
  try {
...
async function collectAgents(codexHome: string, document: TomlDocument): Promise<AgentSummary[]> {
  ...
  for (const section of listAgentSections(document)) {
    ...
    const fileExists = await fileExistsAtPath(resolvedPath);
    agents.push({
      name,
      description,
...
function cloneAgentSectionWithName(section: TomlSection, name: string): TomlSection {
  return {
    name: agentSectionName(name),
    lines: [...section.lines],
  };
}

async function maybeRenameManagedConfigFile(
  codexHome: string,
  originalName: string,
  nextName: string,
  renameManagedFile: boolean,
  nextConfigFile: string | null,
): Promise<{ nextConfigFile: string | null; renamedPaths: RenamedPaths | null }> {
  if (!renameManagedFile || nextName === originalName || !nextConfigFile) {
    return { nextConfigFile, renamedPaths: null };
  }
  ...
  if (managedRelative === newRelative) {
    return { nextConfigFile: newRelative, renamedPaths: null };
  }
  ...
  if (oldExists) {
    ...
    return { nextConfigFile: newRelative, renamedPaths: { oldPath, newPath } };
  }
  return { nextConfigFile: newRelative, renamedPaths: null };
}

function setAgentSectionFields(
  section: TomlSection,
  description: string | null,
  nextConfigFile: string | null,
) {
  setSectionValue(section, "description", description ? JSON.stringify(description) : null);
  setSectionValue(section, "config_file", nextConfigFile ? JSON.stringify(nextConfigFile) : null);
}

async function maybeUpdateManagedInstructions(
  codexHome: string,
  agentName: string,
  nextConfigFile: string | null,
  developerInstructionsInput: string | null | undefined,
  developerInstructions: string | null,
  ...
  if (!nextConfigFile) {
    throw new Error(`agent '${agentName}' does not define config_file; cannot update developer_instructions`);
  }
  const managedRelative = managedRelativePathFromConfig(nextConfigFile);
  if (!managedRelative) {
    throw new Error(
      `agent '${agentName}' config_file is external; edit that file directly to change developer_instructions`,
    );
  }
...
async function restoreAgentUpdateArtifacts(
  configBackup: ConfigBackup | null,
  renamedPaths: RenamedPaths | null,
) {
  if (configBackup) {
  ...
  const newExists = await pathExists(renamedPaths.newPath);
  if (newExists) {
    await fs.rename(renamedPaths.newPath, renamedPaths.oldPath);
  }
}

type UpdateAgentDraft = {
  nextSection: TomlSection;
  nextConfigFile: string | null;
};

type UpdateAgentArtifacts = {
  renamedPaths: RenamedPaths | null;
  configBackup: ConfigBackup | null;
};

function buildUpdateAgentDraft(
  originalSection: TomlSection,
  nextName: string,
): UpdateAgentDraft {
  const nextSection = cloneAgentSectionWithName(originalSection, nextName);
  return {
    nextSection,
    nextConfigFile: readSectionString(nextSection, "config_file"),
  };
}

async function prepareUpdateAgentArtifacts(
  codexHome: string,
  originalName: string,
  nextName: string,
  renameManagedFile: boolean,
  ...
  nextConfigFile: string | null;
  artifacts: UpdateAgentArtifacts;
}> {
  const renameOutcome = await maybeRenameManagedConfigFile(
    codexHome,
    originalName,
    nextName,
    renameManagedFile,
  ...
  const resolvedConfigFile = renameOutcome.nextConfigFile;
  const configBackup = await maybeUpdateManagedInstructions(
    codexHome,
    nextName,
    resolvedConfigFile,
  ...
  return {
    nextConfigFile: resolvedConfigFile,
    artifacts: {
      renamedPaths: renameOutcome.renamedPaths,
      configBackup,
    },
  };
}

function applyUpdateAgentDraft(
  ...
  setAgentSectionFields(draft.nextSection, description, nextConfigFile);
  document.sections.splice(originalSectionIndex, 1, draft.nextSection);
}
...
export async function updateAgent(codexHome: string, input: UpdateAgentInput) {
  ...
  const draft = buildUpdateAgentDraft(originalSection, nextName);
  let nextConfigFile = draft.nextConfigFile;
  let artifacts: UpdateAgentArtifacts = {
    renamedPaths: null,
    configBackup: null,
  };
  try {
    const preparation = await prepareUpdateAgentArtifacts(
      codexHome,
      originalName,
      nextName,
      renameManagedFile,
    ...
    nextConfigFile = preparation.nextConfigFile;
    artifacts = preparation.artifacts;
    applyUpdateAgentDraft(document, originalSectionIndex, draft, description, nextConfigFile);
    await writeConfigContents(codexHome, renderDocument(document));
  } catch (error) {
    await restoreAgentUpdateArtifacts(artifacts.configBackup, artifacts.renamedPaths);
    throw error;
  }
  return await getAgentsSettings(codexHome);
}
...
export async function readAgentConfigToml(codexHome: string, agentName: string) {
  ...
  return await fs
    ...
    .then(
      (content): AgentConfigTomlReadResult => ({
        exists: true,
        content,
      }),
    )
    .catch((error: NodeJS.ErrnoException) => {
      if (error.code === "ENOENT") {
        return {
          exists: false,
          content: null,
        } satisfies AgentConfigTomlReadResult;
      }
