...
describe("agentsConfig", () => {
  ...
  it("creates, updates, reads, writes, and deletes managed agents via config.toml", async () => {
    ...
    await expect(readAgentConfigToml(codexHome, "researcher-v2")).resolves.toMatchObject({
      exists: true,
      content: expect.stringContaining('developer_instructions = "Prefer deterministic fixes."'),
    });
    ...
    await expect(readAgentConfigToml(codexHome, "researcher-v2")).resolves.toMatchObject({
      exists: true,
      content: expect.stringContaining('developer_instructions = "Edited manually."'),
    });
  ...
  it("restores managed agent files when updateAgent fails after forward edits", async () => {
    const codexHome = await createCodexHome();
    await setAgentsCoreSettings(codexHome, {
      multiAgentEnabled: false,
      maxThreads: 6,
    ...
    writeFileSpy.mockImplementation(async (file: any, data: any, options: any) => {
      if (file === configTomlPath) {
    ...
    await expect(fs.readFile(path.join(codexHome, "agents", "researcher.toml"), "utf8")).resolves.toContain(
      'developer_instructions = "Investigate first."',
    );
    await expect(fs.stat(path.join(codexHome, "agents", "researcher-v2.toml"))).rejects.toThrow();
    await expect(fs.readFile(configTomlPath, "utf8")).resolves.toContain('[agents.researcher]');
    await expect(fs.readFile(configTomlPath, "utf8")).resolves.not.toContain("researcher-v2");
  });
  ...
  it("returns an explicit missing-file state for managed configs that have not been created yet", async () => {
    const codexHome = await createCodexHome();
    const settings = await createAgent(codexHome, {
      name: "researcher",
      description: "Research role",
    });
    const managedConfigPath = settings.agents[0]?.resolvedPath;
    expect(managedConfigPath).toBeTruthy();
    await fs.rm(managedConfigPath!, { force: true });

    await expect(readAgentConfigToml(codexHome, "researcher")).resolves.toEqual({
      exists: false,
      content: null,
    });
  });
