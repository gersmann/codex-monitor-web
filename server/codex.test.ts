...
import { handleCompanionRuntimeRpc, type RuntimeRpcContext } from "./codex/codexRpcRuntime.js";
...
describe("CodexCompanionServer phase 1 rpc support", () => {
  ...
  it("includes degraded metadata when list_threads falls back to local cache", async () => {
    const { server } = await createServerFixture();
    const listThreads = vi.fn().mockRejectedValue(new Error("app-server list unavailable"));
    mockAppServerClient(server, { listThreads });
    ...
    expect(result).toMatchObject({
      ...
      degraded: {
        source: "local_cache",
        reason: "app-server list unavailable",
      },
    });
  });
  ...
  it("includes degraded metadata when resume_thread falls back to local cache", async () => {
    const { server, thread } = await createServerFixture();
    const localThread: StoredThread = {
      ...thread,
      id: "sdk-thread-1",
      sdkThreadId: "sdk-thread-1",
    };
    const threadMap = (server as unknown as { threadsById: Map<string, StoredThread> }).threadsById;
    threadMap.clear();
    threadMap.set(localThread.id, localThread);
    const resumeThread = vi.fn().mockRejectedValue(new Error("app-server resume unavailable"));
    mockAppServerClient(server, { resumeThread });
    ...
    expect(result).toMatchObject({
      ...
      degraded: {
        source: "local_cache",
        reason: "app-server resume unavailable",
      },
    });
  });
  ...
  it("rejects rollback targets that are not user messages", async () => {
    ...
    expect(result).toEqual({
      error: {
        status: 400,
        message: "Only user messages can be used as rollback targets.",
      },
    });
  });
  ...
  it("classifies unexpected skills_list failures as internal errors", async () => {
    const { server, storage } = await createServerFixture();
    vi.spyOn(storage, "readSettings").mockRejectedValueOnce(new Error("database offline"));

    await expect(server.handleRpc("skills_list", { workspaceId: "ws-1" })).resolves.toEqual({
      error: {
        status: 500,
  ...
  it("supports direct runtime RPC dispatch for static runtime methods", async () => {
    const context: RuntimeRpcContext = {
      terminalRuntime: null,
      getWorkspace: () => null,
      getLocalUsageSnapshot: async () => ({ ok: true }),
      runCodexDoctor: async () => ({ ok: true }),
      badRequest: (message) => ({ error: { status: 400, message } }),
      notFound: (message) => ({ error: { status: 404, message } }),
    };

    await expect(handleCompanionRuntimeRpc(context, "codex_update", {})).resolves.toEqual({
      ok: false,
      method: "unknown",
      package: null,
      beforeVersion: null,
    ...
    await expect(handleCompanionRuntimeRpc(context, "app_build_type", {})).resolves.toBe("release");
    await expect(handleCompanionRuntimeRpc(context, "send_notification_fallback", {})).resolves.toBeNull();
  });
...
describe("CodexCompanionServer workspace rpc edge cases", () => {
  it("returns notFound for missing workspace removals", async () => {
    const { server } = await createServerFixture();

    await expect(server.handleRpc("remove_workspace", { id: "missing" })).resolves.toEqual({
      error: {
        status: 404,
        message: "Workspace not found.",
      },
    });
    await expect(server.handleRpc("remove_worktree", { id: "missing" })).resolves.toEqual({
      error: {
        status: 404,
        message: "Workspace not found.",
      },
  ...
  it("returns notFound for missing workspace list/read rpc methods", async () => {
    const { server } = await createServerFixture();

    await expect(server.handleRpc("list_workspace_files", { workspaceId: "missing" })).resolves.toEqual({
      error: {
        status: 404,
        message: "Workspace not found.",
      },
    });
    await expect(server.handleRpc("list_git_branches", { workspaceId: "missing" })).resolves.toEqual({
      error: {
        status: 404,
        message: "Workspace not found.",
      },
    });
    await expect(server.handleRpc("list_git_roots", { workspaceId: "missing", depth: 2 })).resolves.toEqual({
      error: {
        status: 404,
        message: "Workspace not found.",
      },
    });
    await expect(server.handleRpc("get_git_remote", { workspaceId: "missing" })).resolves.toEqual({
      error: {
        status: 404,
        message: "Workspace not found.",
      },
  ...
  it("returns badRequest for inaccessible add_workspace paths", async () => {
    const { server } = await createServerFixture();
    const missingPath = path.join(os.tmpdir(), `codex-monitor-missing-${Date.now()}`);

    await expect(server.handleRpc("add_workspace", { path: missingPath })).resolves.toEqual({
      error: {
        status: 400,
  ...
  it("rejects prompt paths outside allowed roots even when they share a prefix", async () => {
    const { dir, server } = await createServerFixture();
    const siblingPromptsDir = path.join(dir, "prompts-evil");
    const siblingPromptPath = path.join(siblingPromptsDir, "trap.md");
    await fs.mkdir(siblingPromptsDir, { recursive: true });
    await fs.writeFile(siblingPromptPath, "malicious", "utf8");

    await expect(
      server.handleRpc("prompts_delete", {
        workspaceId: "ws-1",
        path: siblingPromptPath,
      }),
    ).resolves.toEqual({
      error: {
        status: 400,
  ...
  it("reads image data URLs only from paths within the workspace root", async () => {
    const { dir, server, workspace } = await createServerFixture();
    await fs.mkdir(workspace.path, { recursive: true });
    const imagePath = path.join(workspace.path, "screenshot.png");
    await fs.writeFile(imagePath, Buffer.from([0x89, 0x50, 0x4e, 0x47]));

    const allowed = await server.handleRpc("read_image_as_data_url", {
      workspaceId: "ws-1",
      path: imagePath,
    });
    await expect(
      server.handleRpc("read_image_as_data_url", {
        workspaceId: "ws-1",
        path: path.join(dir, "outside.png"),
      }),
    ).resolves.toEqual({
      error: {
        status: 400,
    ...
    expect(allowed).toMatch(/^data:image\/png;base64,/);
  });

  it("handles unsupported runtime-only methods in the extracted dispatcher", async () => {
    const context: RuntimeRpcContext = {
      terminalRuntime: null,
      getWorkspace: () => null,
      getLocalUsageSnapshot: async () => null,
      runCodexDoctor: async () => ({ ok: true }),
      badRequest: (message) => ({ error: { status: 400, message } }),
      notFound: (message) => ({ error: { status: 404, message } }),
    };
    ...
    await expect(
      handleCompanionRuntimeRpc(context, "menu_set_accelerators", {}),
    ).resolves.toEqual({
      error: {
        status: 400,
