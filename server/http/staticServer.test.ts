...
describe("server/http/staticServer", () => {
  ...
  it("returns null for malformed percent-encoded paths", async () => {
    const root = await makeStaticRoot();
    await writeFile(path.join(root, "index.html"), "<html>shell</html>");

    const filePath = await resolveStaticFile(root, "/%E0%A4%A");
    const response = await readStaticResponse(root, "/%E0%A4%A");

    expect(filePath).toBeNull();
    expect(response).toBeNull();
  });

  it("returns null when the resolved file cannot be read", async () => {
    const root = await makeStaticRoot();
    const filePath = path.join(root, "index.html");
    await writeFile(filePath, "<html>shell</html>");
