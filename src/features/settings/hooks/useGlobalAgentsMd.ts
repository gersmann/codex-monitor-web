...
const EMPTY_STATE: GlobalAgentsState = {
  content: "",
  exists: false,
  truncated: false,
  isLoading: false,
...
export function useGlobalAgentsMd() {
  const [state, setState] = useState<GlobalAgentsState>(EMPTY_STATE);
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      const response = await readGlobalAgentsMd();
      ...
      pushErrorToast({
        title: "Couldn’t load global AGENTS.md",
        message,
      });
    } finally {
      // Always clear the in-flight flag. A superseded request (e.g., after save)
      // would otherwise leave refresh permanently blocked.
  ...
  const save = useCallback(async () => {
    ...
    try {
      await writeGlobalAgentsMd(content);
      if (requestId !== requestIdRef.current) {
        return false;
      }
      ...
      pushErrorToast({
        title: "Couldn’t save global AGENTS.md",
        message,
      });
      return false;
    }
